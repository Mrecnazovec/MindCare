				...
				<div className='flex md:hidden justify-center mt-8'>
					{Array.from({ length: count2 }).map((_, index) => (
						<button
							...
							key={index}
							className={`w-3 h-3 transition-all duration-300 cursor-pointer ${
								current2 === index ? 'bg-main w-4' : 'bg-gray-400 mr-2 rounded-full'
							} ${current2 === index && 'rounded-l-full'} ${current2 === index && 'rounded-r-full mr-2'}`}
						></button>
					))}
				</div>
			</Carousel>
